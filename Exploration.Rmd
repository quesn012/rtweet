...
w= 8
thresh = 0.01 #was .015
Terms12_nostem2 <- terms(results, w,thresh) 
...
Not every topic in our set was a winner... this is an example of that.

"Liberal Media (CNN/Washington Post) Related to Trump"
```{r}
Terms12_nostem[9:16] 
top[2] = 'Liberal Media Trump'
```

Overeducated conservative cowboy. Convservatexan is a twitter handle
...
This topic is relatively simple; focused on trump slogans like #MAGA and including Trump's actual twitter handle. This a pro-Trump hash tag topic.
...
top[9] = 'Imperative call for action'
```
...
top[12] = 'Imperative call for action'
```
...
First let's look at volume by topic.
...
More relevant than by topic I think would be looking at how the topics adjusted over time and what topics were being pushed during whats points in the election.
...
If we can get a way to only keep non-sparse items by month in here... we should be good.
  tfm$dimnames$docs has the id of the docs we kept. we can use that to filter to our original docs to get the list of months
